    ...
    fn flash_write(&mut self, addr: u64, data: &[u8]) -> gdbstub::target::TargetResult<(), Self> {
        ...
        if let Some(high) = self.flash_high_water {
            if addr < high {
                ...
                // The dropped data's high-water mark is stale too: this
                // write is the start of the new load, so the mark must
                // restart from it - otherwise every following packet of
                // a retried load lands below the old mark and wipes the
                // data it just staged.
