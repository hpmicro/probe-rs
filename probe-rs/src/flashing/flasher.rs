...
impl<'session> Flasher<'session> {
    ...
    fn program_double_buffer(&mut self, flash_encoder: &FlashEncoder) -> Result<(), FlashError> {
        ...
        let result = self.run_program(|active| {
            ...
            // Two batch-sized strips: while the routine programs from
            // one strip, the next batch's data is transported into the
            // other - the transport overlaps the physical write. The
            // system bus reaches the data memory independently of the
            // hart, so the writes race the routine instead of waiting
            // for it.
            let strip_base = active.buffer_address(0);
            let strips = [strip_base, strip_base + (BATCH_PAGES * 256) as u64];
